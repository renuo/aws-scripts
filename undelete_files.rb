...
versions = prefixes.map do |prefix|
  result = `aws s3api list-object-versions --profile #{profile} --bucket #{bucket} --prefix #{prefix} --query 'DeleteMarkers[].{Key: Key, VersionId: VersionId}'`
  JSON.parse(result) unless result.empty?
end.flatten.compact

delete_request_skeleton = { Bucket: 'heleda-master', Delete: { Objects: versions } }

puts `aws s3api delete-objects \
  --bucket heleda-master \
  --region eu-central-1 \
  --profile info@renuo.ch \
  --cli-input-json '#{JSON.generate(delete_request_skeleton)}'`
